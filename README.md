...
默认查看地址：`http://127.0.0.1:3847`（**单实例**：端口占用时结束旧进程并复用同一端口，不递增）
...
只要本回合会创建/修改/删除代码或项目文件，必须使用 skill `codex-session-diff-viewer`，
展示**本次会话变更**，禁止用 git 工作区相对仓库的 diff 代替。
...
- 本地始终只保留 **一个** 查看器进程
- 再次 `open` 时会：
  1. 结束所有本项目旧 `server.js` 进程（含历史递增端口残留）
  2. 复用同一端口 `3847` 启动新实例
- **不会**自动 `3847 → 3848 → 3849...`，避免反复 open 造成多进程资源浪费
...
会先结束占用 `3847` 的旧查看器进程，再复用同一端口启动。  
正常情况下地址始终是：`http://127.0.0.1:3847`。  
若仍无法释放端口，请手动检查：
